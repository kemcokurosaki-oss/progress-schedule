...
function renderTable() {
    ...
    list.forEach(row => {
        ...
        html += `<tr class="${row.overall === "delayed" ? "row-delayed" : ""}" data-pn="${escapeHtml(row.project_number)}">
            ...
            ${STAGES.map((s, i) => stageCellHtml(row, s, s.isInspection && !STAGES[i - 1].isInspection)).join("")}
            ${otherCellHtml(row)}
        </tr>`;
        if (isExpanded) {
...
function buildTableHead() {
    ...
    let html = `
        ...
        ${STAGES.map((s, i) => `<th class="stage-head${s.isInspection ? " stage-head-inspect" : ""}${s.isInspection && !(STAGES[i - 1] && STAGES[i - 1].isInspection) ? " stage-head-inspect-first" : ""}" title="${s.isInspection ? "点検案件専用の工程列" : ""}">${s.label}</th>`).join("")}
